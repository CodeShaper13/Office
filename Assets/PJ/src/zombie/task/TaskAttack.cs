...
public class TaskAttack : TaskBase {
    ...
    public override bool preform() {
        if(this.target == null) {
            ...
            // No set target, search for a player.
            List<Player> players = Main.singleton.getPlayerManager().getAllPlayers<Player>().OrderBy(x => Vector2.Distance(this.zombie.transform.position, x.transform.position)).ToList();
            Player target = null;
